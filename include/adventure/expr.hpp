...
template<class Op, class L, class R, class T> struct BinExpr;
template<class Op, class E, class T> struct UnaryExpr;

template<class Derived, class T>
struct ExprBase {
    using scalar_type = T;
    constexpr ADVENTURE_STRONG_INLINE T value() const { return static_cast<const Derived&>(*this).value_impl(); }
...
template<class E> struct parent_count; // primary template (undefined)
...
struct parent_count<BinExpr<Op, L, R, T>>
    : std::integral_constant<std::size_t,
          parent_count<L>::value + parent_count<R>::value> {};
...
struct ConstExpr : ExprBase<ConstExpr<T>, T> {
    ...
    template<class Writer>
    constexpr ADVENTURE_STRONG_INLINE void derivative_impl(Writer&&) const noexcept {}
};
...
struct BinExpr : ExprBase<BinExpr<Op, L, R, T>, T> {
    ...
    template<class Writer>
    constexpr ADVENTURE_STRONG_INLINE void derivative_impl(Writer&& w) const noexcept {
        struct ScaledWriter {
            Writer& w_;
            T factor_;
            constexpr ADVENTURE_STRONG_INLINE void operator()(index_t parent, T coeff) const noexcept {
...
template<class T>
constexpr ADVENTURE_STRONG_INLINE auto as_expr(T v) noexcept { return ConstExpr<T>(v); }
...
struct UnaryExpr : ExprBase<UnaryExpr<Op, E, T>, T> {
    ...
    constexpr ADVENTURE_STRONG_INLINE T value_impl() const noexcept { return Op::template value<T>(v_); }

    template<class Writer>
    constexpr ADVENTURE_STRONG_INLINE void derivative_impl(Writer&& w) const noexcept {
        if constexpr (parent_count<E>::value != 0) {
            T factor = Op::derivative(v_);
            auto scaled = [&](index_t parent, T coeff) {
                w(parent, coeff * factor);
            };
...
constexpr ADVENTURE_STRONG_INLINE auto operator+(const ExprBase<L, T>& l, const ExprBase<R, T>& r) {
...
template<class L, class R, class T>
constexpr ADVENTURE_STRONG_INLINE auto min(const ExprBase<L, T>& l, const ExprBase<R, T>& r) {
...
template<class L, class R, class T>
constexpr ADVENTURE_STRONG_INLINE auto max(const ExprBase<L, T>& l, const ExprBase<R, T>& r) {
...
constexpr ADVENTURE_STRONG_INLINE auto clamp(const ExprBase<X, T>& x, const ExprBase<L, T>& lo, T hi) {
    return clamp(x, lo, as_expr(hi));
}
template<class X, class H, class T>
constexpr ADVENTURE_STRONG_INLINE auto clamp(const ExprBase<X, T>& x, T lo, const ExprBase<H, T>& hi) {
    return clamp(x, as_expr(lo), hi);
}
template<class X, class T>
constexpr ADVENTURE_STRONG_INLINE auto clamp(const ExprBase<X, T>& x, T lo, T hi) {
    return clamp(x, as_expr(lo), as_expr(hi));
}
template<class L, class H, class T>
constexpr ADVENTURE_STRONG_INLINE auto clamp(T x, const ExprBase<L, T>& lo, const ExprBase<H, T>& hi) {
    return clamp(as_expr(x), lo, hi);
}
template<class L, class T>
constexpr ADVENTURE_STRONG_INLINE auto clamp(T x, const ExprBase<L, T>& lo, T hi) {
    return clamp(as_expr(x), lo, as_expr(hi));
}
template<class H, class T>
constexpr ADVENTURE_STRONG_INLINE auto clamp(T x, T lo, const ExprBase<H, T>& hi) {
    return clamp(as_expr(x), as_expr(lo), hi);
}

template<class L, class R, class T>
constexpr ADVENTURE_STRONG_INLINE auto pow(const ExprBase<L, T>& l, const ExprBase<R, T>& r) {
...
template<class L, class R, class T>
constexpr ADVENTURE_STRONG_INLINE bool operator==(const ExprBase<L, T>& l, const ExprBase<R, T>& r) {
...
struct EdgeWriter {
    ...
    explicit EdgeWriter(Tape<T>& t) noexcept
        : tape(t), start(t.edges.size()) {}

    ADVENTURE_STRONG_INLINE void operator()(typename Tape<T>::index_type parent, T coeff) noexcept {
        if (coeff == T(0)) return;
...
template<class Expr>
ADVENTURE_STRONG_INLINE Variable<typename Expr::scalar_type> materialise(const Expr& e) noexcept {

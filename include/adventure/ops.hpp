...
struct AddOp {
  template <class T>
  static constexpr T coeff_left(const T &, const T &) {
    return T(1);
  }
  template <class T>
  static constexpr T coeff_right(const T &, const T &) {
    return T(1);
  }
  template <class T>
  static constexpr T value(const T &a, const T &b) {
    return a + b;
  }
};
...
struct NegOp {
  ...
  template <class T>
  static constexpr T derivative(const T &) {
    return T(-1);
  }
};
